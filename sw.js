/* Service worker: permite deschiderea aplicației și fără semnal (Firestore are propriul cache de date). */
const CACHE = 'receptii-v1';
...
self.addEventListener('fetch', e => {
  ...
  // Firestore / Auth traffic: never intercept.
  if (url.hostname.endsWith('googleapis.com') || url.hostname.endsWith('firebaseio.com')) return;
  // SDK scripts and fonts: cache-first (they are versioned).
  ...
  // Own files: network-first, fallback to cache (so updates arrive, but offline still opens).
